...
namespace Unity.Burst
{
...
#endif
    {
        ...
        internal static string SerialiseStringArraySafe(string[] array)
        {
            var s = new StringBuilder();
            foreach (var entry in array)
            {
                s.Append($"{entry.Length}]");
                s.Append(entry);
            }
            return s.ToString();
        }

        internal static string[] DeserialiseStringArraySafe(string input)
        {
            ...
            while (index < length)
            {
                var nextItem = input.IndexOf(']', index);
                var len = Int32.Parse(input.Substring(index, nextItem - index));
                listFolders.Add(input.Substring(nextItem + 1, len));
                index = nextItem + 1 + len;
            }
        ...
        internal static string SerialiseCompilationOptionsSafe(string[] roots, string[] folders, string options)
        {
            ...
            finalSerialise[0] = SerialiseStringArraySafe(roots);
            finalSerialise[1] = SerialiseStringArraySafe(folders);
            finalSerialise[2] = options;
            return SerialiseStringArraySafe(finalSerialise);
        }

        internal static (string[] roots, string[] folders, string options) DeserialiseCompilationOptionsSafe(string from)
        {
            var set = DeserialiseStringArraySafe(from);

            return (DeserialiseStringArraySafe(set[0]), DeserialiseStringArraySafe(set[1]), set[2]);
        }
...
#if UNITY_EDITOR
                    BurstCompiler.IsScriptDebugInfoEnabled ? "Full" : "LineOnly"
#else
                    "LineOnly"
#endif
                ));
