...
public class BurstInspectorGUITests
{
    ...
    private InfoThingy SetupBranchTest()
    {
        ...
        var mousePos = new Vector2(x, topOfInspectorToBranchArrow + window._inspectorView.height - window._textArea.fontHeight);
    ...
    [UnityTest]
    public IEnumerator FocusCodeNotBranchesTest()
    {
        const string case1 = "BurstInspectorGUITests.MyJob - (IJob)";
        const string case2 = "BurstInspectorGUITests.MyJob2 - (IJob)";

        var win = EditorWindow.GetWindow<BurstInspectorGUI>();

        // Force window size to be small enough for it to position it more to the right.
        win.position = new Rect(win.position.x, win.position.y, 390, 405);
